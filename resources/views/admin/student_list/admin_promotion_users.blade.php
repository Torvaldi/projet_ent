...
@section('content')
    <div class="container">
        ...
        <h3>Liste des utilisateur du groupe {{ $promotion->name }}</h3>
                ...
                <div class="tab-pane fade" id="{{ $semester->id }}" role="tabpanel" aria-labelledby="{{ $semester->name }}">
                    <table class="table mt-3">
                        ...
                        @foreach($semester->users as $user)
                            @if(($user->tpgroup->tdgroup->promotion->id == $promotion->id) && !($user->hasRole('Administrateur')) && !($user->hasRole('Professeur')))
                                <tr>
                                    <td>{{ $user->lastName }}</td>
